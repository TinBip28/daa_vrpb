...
def visualize_vrpb_solution(instance_data, all_tours_nodes, total_distance, title="VRPB Solution"): # THÊM total_distance
    """
    Vẽ đồ thị các lộ trình VRPB.
    - instance_data: dictionary chứa thông tin instance (coords, num_linehaul, num_nodes)
    - all_tours_nodes: list các lộ trình, mỗi lộ trình là một list các node_index.
    - total_distance: Tổng quãng đường của giải pháp.
    ...
    plt.figure(figsize=(13, 10)) # Tăng kích thước một chút để có không gian cho text

    # 1. Vẽ các điểm (nodes)
    plt.plot(coords[depot_idx, 0], coords[depot_idx, 1], 's', markersize=10, color='red', label='Depot (0)')
    ...
    if num_total_customers > num_linehaul:
        for i in range(num_linehaul + 1, num_total_customers + 1): # Index từ 1 đến num_total_customers
    ...
    if len(all_tours_nodes) > len(route_colors):
        try:
    ...
    for i, tour_nodes in enumerate(all_tours_nodes):
        ...
        tour_coords_x = [coords[node_idx, 0] for node_idx in tour_nodes]
        tour_coords_y = [coords[node_idx, 1] for node_idx in tour_nodes]
        ...
        for j in range(len(tour_nodes) - 1):
            ...
            # Kiểm tra index trước khi truy cập coords
    ...
    # THÊM HIỂN THỊ TỔNG QUÃNG ĐƯỜNG VÀO TIÊU ĐỀ
    full_title = f"{title}\nTổng quãng đường: {total_distance:.2f} | Số tours: {len(all_tours_nodes)}"
